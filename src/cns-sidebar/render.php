...
$top_level = array_values( array_filter( $items, fn( $i ) => empty( $i['parentId'] ) ) );
...
$render_item = function( array $item, array $all_items ) use ( &$render_item ): string {
    $is_group = ( $item['itemType'] ?? 'link' ) === 'group';
    $children = array_values( array_filter( $all_items, fn( $i ) => ( $i['parentId'] ?? '' ) === $item['id'] ) );
    ...
    if ( $is_group ) {
        ...
        $html .= '<span class="cns-sidebar__group-arrow">▸</span>' . $label;
        $html .= '</button>';

        if ( $children ) {
            $html .= '<ul class="cns-sidebar__sub-links" data-wp-class--is-open="context.isGroupOpen">';
    ...
    // Regular link
    $target = ! empty( $item['linkNewTab'] ) ? ' target="_blank" rel="noopener noreferrer"' : '';
...
$context = esc_attr( wp_json_encode( [ 'isOpen' => false, 'isMobileOpen' => false ] ) );
?>
<aside
...
>
  ...
  <div class="cns-sidebar__panel">
    ...
    >
      ...
      <span class="cns-sidebar__mobile-arrow">▼</span>
    </button>
    <?php endif; ?>
